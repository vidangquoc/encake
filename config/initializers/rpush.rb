...
Rpush.reflect do |on|
  ...
  on.gcm_canonical_id do |old_id, canonical_id|
    DeviceKey.change_key('android', old_id, canonical_id)
    AppLog.log("Android canonical id changes: #{old_id} -> #{canonical_id}", 'rpush')
  end
  ...
  on.gcm_invalid_registration_id do |app, error, registration_id|
    DeviceKey.delete_key('android', registration_id)
    AppLog.log("Android invalid registration id, error: #{error.inspect}, registration_id: #{registration_id}", 'rpush')
  end
